...
import { SessionVerboseOutput } from '@/features/session-detail/SessionVerboseOutput'
  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between">
        <div>
          <Link to="/sessions" className="text-xs text-gray-500 hover:text-gray-300">
            &larr; Sessions
          </Link>
          <h1 className="mt-1 text-xl font-bold text-gray-100">
            {privacyMode ? anonymizeProjectName(detail.projectName) : detail.projectName}
          </h1>
          <div className="mt-1 flex items-center gap-3 text-xs text-gray-400">
            {detail.branch && <span className="font-mono">{anonymizeBranch(detail.branch)}</span>}
          ...
          {detail.models.length > 0 && (
            <div className="mt-1 flex gap-1">
              {detail.models.map((m) => (
                <span key={m} className="rounded bg-gray-800 px-1.5 py-0.5 text-[10px] font-mono text-gray-400">
      ...
      <div className="mt-6">
        <SessionVerboseOutput sessionId={sessionId} projectPath={project} />
      </div>

      <div className="mt-6">
        <h2 className="mb-3 text-sm font-semibold text-gray-300">Timeline</h2>
        <TimelineEventsChart turns={detail.turns} agents={detail.agents} skills={detail.skills} errors={detail.errors} />
      </div>
